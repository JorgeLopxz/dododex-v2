...
/** null = 404 real (no existe); undefined = fallo transitorio tras reintentos (no pisar dato previo) */
async function fetchMapPage(title, retries = 5) {
  ...
  for (let i = 0; i <= retries; i++) {
    ...
    try {
      res = await fetch(url, {
        headers: {
          // OJO: la wiki devuelve 403 con user-agents recortados — usar UA completo estilo navegador
          'user-agent':
      ...
      if (i < retries) await sleep(5000 * (i + 1))
      continue
    }
    if (res.ok) return await res.text()
    if (res.status === 404) return null
    if (i < retries) await sleep(5000 * (i + 1)) // 429/403: backoff 5,10,15,20,25s
  }
  console.warn(`  ⚠ sin datos tras reintentos: ${title}`)
  return undefined
}
...
function transform(raw, label, fallbackStatsByBp = new Map(), { asaOnly = false } = {}) {
  ...
  const UNTAMEABLE_NAME =
    /^(Alpha |Skeletal |Zombie |Eerie |Enraged |Malfunctioned |VR |Bone |Party |Bunny |Revenant )|(Corrupted|Ghost|DodoRex|Dodo Wyvern|Zomdodo|Murder Turkey|Insect Swarm|Diseased|Broodmother|Yeti|\bDrone\b|Defense Unit|Scout|Enforcer|^Mek$|Exo-Mek|Macrophage|Summoner|Forest Wyvern|Rubble Golem|Titan$|Titan Flock|Lamprey|Subterranean Reaper|Surface Reaper|Elemental Reaper|Reaper Queen|Minion|Tamed\))/
  ...
  const isUntameable = (sp) => {
    ...
    return (
      ...
      // R- (Genesis 2) y demás contenido Gen2: legítimo en ASE, aún no liberado en ASA
      (asaOnly && (/^R-/.test(n) || UNRELEASED_BP.test(sp.blueprintPath ?? '')))
    )
  }
...
async function buildMapData() {
  // Partimos del JSON previo: si una petición falla por rate-limit, conservamos lo ya bueno
  const prevPath = `${OUT_DIR}/wiki-maps.json`
  const prev = existsSync(prevPath) ? JSON.parse(readFileSync(prevPath, 'utf8')) : { asa: {}, ase: {} }
  const out = { asa: {}, ase: {} }
  for (const version of /** @type {const} */ (['asa', 'ase'])) {
    for (const mapName of MAPS[version]) {
      const before = prev[version]?.[mapName]
      // ASE: página base. ASA: página /ASA FUSIONADA con la base — la /ASA solo trae los
      // grupos re-verificados (Island/ASA: 6) y varios mapas ni la tienen; la base completa.
      const base = parseMapData(await fetchMapPage(`Data:Maps/Resources/${mapName}`))
      await sleep(1500)
      const asaRes = version === 'asa' ? parseMapData(await fetchMapPage(`Data:Maps/Resources/${mapName}/ASA`)) : null
      await sleep(1500)
      // Capas: previo (bueno) → base ASE → overrides ASA. Nunca degradamos por rate-limit;
      // los extras ASA (p.ej. gemas de Ragnarok/ASA) se superponen si la /ASA responde.
      let groups = { ...(before?.groups ?? {}) }
      if (base && Object.keys(base.groups).length) groups = { ...groups, ...base.groups }
      if (version === 'asa' && asaRes && Object.keys(asaRes.groups).length) groups = { ...groups, ...asaRes.groups }

      let spawns = null
      if (version === 'asa') {
        spawns = parseSpawns(await fetchMapPage(`Data:Spawn Map/${mapName}/ASA`))
        if (!spawns) {
          await sleep(1500)
          spawns = parseSpawns(await fetchMapPage(`Data:Spawn Map/${mapName}`))
        }
      } else {
        spawns = parseSpawns(await fetchMapPage(`Data:Spawn Map/${mapName}`))
      }
      ...
      if ((!spawns || spawns.length === 0) && before?.spawns?.length > 0) spawns = before.spawns

      out[version][mapName] = { groups, spawns: spawns ?? [] }
      console.log(
        `${version.toUpperCase()} ${mapName}: ${Object.keys(groups).length} grupos, ${(spawns ?? []).length} spawns`,
      )
    }
  }
